/*
 * Starter file for Project 2 Part 1 (https://edstem.org/au/courses/31090/lessons/106820/slides/738872)
 *
...
void generate_lantern_paths(map_t* map){
	int size = map->num_lanterns;
	lantern_t *lantern = map->lantern;
	
	if (lantern[0].arrival_time < abs(lantern[0].x - map->drone[0].x) && size < 2) {
		return;
	} 
	...
	for (int i = 0; i < size; i++) {
		if (lantern[i].arrival_time >= abs(lantern[i].x - map->drone[0].x)) {
			dp[i] = 1;
			parent[i] = -1;
		}

		for (int j = 0; j < i; j++) {
			if (lantern[i].arrival_time - lantern[j].arrival_time >=
			abs(lantern[i].x - lantern[j].x)) {
				if (dp[j] + 1 > dp[i]) {
					dp[i] = dp[j] + 1;
					parent[i] = j;
				}
		...
		maxAt = dp[i] > dp[maxAt] ? i : maxAt;
	}
	
	
	int *temp = &map->drone[0].path_length;
	*temp = dp[maxAt];
	for (int i = *temp - 1; i >= 0; i--) {
		map->drone[0].lantern_path[i] = maxAt;
		maxAt = parent[maxAt];
	}
